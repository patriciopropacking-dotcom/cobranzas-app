  ...
  return (
    <div>
      ...
      {(alertInvs.length > 0 || toContact.length > 0) && (
        <div className="alert-grid" style={{ display:"grid", gridTemplateColumns: alertInvs.length>0 && toContact.length>0 ? "1fr 1fr" : "1fr", gap:16, marginBottom:20 }}>
          {alertInvs.length > 0 && (
...
export default function App() {
  ...
  function navigate(id) {
    setView(id);
    if (id !== "clients") setSelectedClientId(null);
  ...
  return (
    <div style={{ minHeight:"100vh", background:bg, color:navy, fontFamily:font, display:"flex" }}>
        ...
        <div className="sidebar-mobile-overlay" style={{ position:"fixed", inset:0, zIndex:50, display:"flex" }}>
          <div style={{ width:260, background:navy, display:"flex", flexDirection:"column", minHeight:"100vh" }}>
            <div style={{ padding:"20px 20px 18px", borderBottom:"1px solid rgba(255,255,255,.08)", display:"flex", alignItems:"center", gap:12 }}>
              ...
              <button onClick={() => setSidebarOpen(false)} style={{ background:"none", border:"none", color:"rgba(255,255,255,.5)", fontSize:24, cursor:"pointer", padding:0, lineHeight:1 }}>×</button>
            </div>
            <div style={{ padding:"12px 20px", borderBottom:"1px solid rgba(255,255,255,.08)" }}>
              <div style={{ fontSize:10, color:"rgba(255,255,255,.35)", textTransform:"uppercase", letterSpacing:1.5, marginBottom:4 }}>Empresa activa</div>
              <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between" }}>
      ...
      <div className="main-content" style={{ marginLeft:220, flex:1, display:"flex", flexDirection:"column", minHeight:"100vh" }}>
        ...
        {/* Topbar */}
        <div className="topbar-padding" style={{ background:white, borderBottom:`1px solid ${bc}`, padding:"0 32px", height:56, display:"flex", alignItems:"center", justifyContent:"space-between", position:"sticky", top:0, zIndex:20 }}>
          <div style={{ display:"flex", alignItems:"center", gap:12 }}>
            {/* Hamburger */}
            <button className="hamburger" onClick={() => setSidebarOpen(true)}
              style={{ background:"none", border:"none", cursor:"pointer", padding:4, color:navy, fontSize:22, lineHeight:1, display:"flex", alignItems:"center" }}>☰</button>
            <div>
              <div style={{ fontSize:16, fontWeight:700, color:navy }}>
                {navItems.find(n=>n.id===view)?.label || "Dashboard"}
              </div>
              <div className="hide-mobile" style={{ fontSize:11, color:muted, marginTop:1 }}>
        ...
        {/* Content */}
        <main className="main-padding" style={{ padding:"24px 32px", flex:1 }}>
